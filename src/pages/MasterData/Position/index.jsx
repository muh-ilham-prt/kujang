...
export default function Position() {
  ...
  const handleDelete = () => {
    setPositions((prev) =>
      prev.filter((p) => p.mep_empy_position_id !== toDelete.mep_empy_position_id)
    );
          ...
          <tbody>
            {pageItems.map((position, index) => (
              ...
              >
                ...
                <td className="px-6 py-3">{position.mep_empy_position_short}</td>
                {canSeeEntities && (
                  <td className="px-6 py-3">{entityNames(position.entities)}</td>
                )}
